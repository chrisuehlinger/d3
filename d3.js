!function() {
  ...
  d3.geo.jsonSource = function(sink) {
    return function(object) {
      (object && d3_geo_jsonSourceObjectType.hasOwnProperty(object.type) ? d3_geo_jsonSourceObjectType[object.type] : d3_geo_jsonSourceGeometry)(object, sink);
    };
  };
  ...
  d3.geo.project = function(f, δ, sink) {
    ...
    function segment(x0, y0, λ0, a0, b0, c0, x1, y1, λ1, a1, b1, c1, depth) {
      ...
      if (d2 > 4 * δ2 && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), φ2 = Math.asin(c /= m), λ2 = abs(abs(c) - 1) < ε || abs(λ0 - λ1) < ε ? (λ0 + λ1) / 2 : Math.atan2(b, a), p = f(λ2, φ2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
        if (dz * dz / d2 > δ2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < d3_geo_projectCosMinDistance) {
...
>>>>>>> Expose d3.geo.clipCircle.
  ...
  d3.geo.conicEqualArea = function(φ0, φ1) {
  ...
  d3.geo.satellite = function(P, ω) {
    ...
    return function(λ, φ) {
      var coordinates = vertical(λ, φ), x = coordinates[0], y = coordinates[1], A = y * sinω / (P - 1) + cosω;
      return [ x * cosω / A, y / A ];
    };
  };
...
>>>>>>> Add geom.jsonSource and geom.boundsSink.
  ...
  d3.geom.clipExtent = function(x0, y0, x1, y1, sink) {
    var sink_ = sink, bufferSink = d3_geo_clipBufferSink(), clipLine = d3_geom_clipLine(x0, y0, x1, y1), segments, polygon, ring;
    var clip = {
      point: point,
      lineStart: lineStart,
      lineEnd: lineEnd,
